 ...
 *
 * One ask never happens at all: an `edit` whose target the session never
 * observed is never intercepted — the native tool's read-first gate
 * (`fs/edit-intent`) answers instantly instead, so no approval card is spent
 * on a call that is already certain to fail the moment it is allowed. The
 * gate is probed read-only through the same decision slot (pure lookup, no
 * state added here); a wrong probe degrades toward the ask, never toward
 * silence. `write` is exempt: its degrade-to-createIfAbsent behavior is real
 * work, and write-creates are this plugin's core value.
 *
...
/**
 * Probe the SAME `fs/edit-intent` decision slot the shipped edit consults —
 * dispatched with the identical target and `exec` actor, so the answer comes
 * from the very state the real check will use (fs-observation-policy's
 * listener is a pure WeakMap lookup; only the separate `fs/observed` event
 * ever records state, so this probe is side-effect-free). True only when the
 * gate is certain to refuse the edit (`FS_NOT_OBSERVED` never-read /
 * `FS_NOT_FOUND` confirmed-absent): a resolved `{version}` means observed, a
 * resolved `undefined` means no gate is composed (nothing can bounce the
 * call), and any unexpected failure means "cannot predict". Every non-doomed
 * verdict routes toward the ask, never toward silence.
 */
async function gateRefusesUnobservedEdit(context, target, exec) {
  let gateVerdict = 'unknown'
  try {
    const intent = await context.waterfall('fs/edit-intent', target, exec, () => undefined)
    gateVerdict = intent !== null && typeof intent === 'object' ? 'observed' : 'absent-gate'
  } catch (error) {
    const code = error !== null && typeof error === 'object' ? error.code : undefined
    if (code === 'FS_NOT_OBSERVED' || code === 'FS_NOT_FOUND') gateVerdict = 'doomed'
    // anything else stays 'unknown' — never skip on an unexpected failure
  }
 ...
 *
 * One ask is skipped outright (edit only): when the observation gate would
 * refuse the call anyway, intercepting it would waste an approval card on a
 * mutation that can never run. See `gateRefusesUnobservedEdit` below.
 */
async function planMutation(context, toolName, filePath, exec) {
  ...
  if (toolName === 'edit' && exec.agent !== undefined && await gateRefusesUnobservedEdit(context, target, exec)) {
    // Never intercept a call the observation gate is certain to refuse: run
    // the native path so the tool's own gate answers instantly with its
    // honest read-first error and no approval is requested. This is a
    // routing decision only — the gate stays the single authority (it
    // re-runs inside the real call, whose error surfaces exactly as the
    // shipped tool's would; the fence-denial fallback cannot fire because
    // the gate throws before the fence is reached and with a different
    // code than FS_SANDBOX_DENIED).
    return {
      target,
      standingPolicy,
      approvalRequired: false,
